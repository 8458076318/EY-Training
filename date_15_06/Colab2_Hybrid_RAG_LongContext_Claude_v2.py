...
def context_precision(query: str, relevant_title: str, top_k: int | None = None) -> tuple[float, list[str]]:
    """
...
# %%
# Combined view: tokens, context precision, answer relevance, faithfulness, and latency.
rag_result_by_question = {q: r for q, r in zip(TEST_QUESTIONS, rag_results)}

context_eval_rows = []
for query, relevant_title in CONTEXT_PRECISION_SET:
    rag_res = rag_result_by_question[query]
    reference = REFERENCE_ANSWERS[query]
    quality = quality_metrics(reference, rag_res.answer, rag_res.context)
    context_eval_rows.append({
        "Query": query,
        "Relevant": relevant_title,
        "Total_Tokens": rag_res.input_tokens + rag_res.output_tokens,
        "Context_Precision": next(
            row["Precision"] for row in context_precision_rows if row["Query"] == query
        ),
        "Answer_Relevance": float((quality["BLEU"] + quality["ROUGE-L"]) / 2.0),
        "Faithfulness": quality["Faithfulness"],
        "Latency_ms": rag_res.total_ms,
    })

context_eval_df = pd.DataFrame(context_eval_rows)
print("\nCONTEXT EVAL SUMMARY (tokens / precision / relevance / latency)")
print(context_eval_df[[
    "Query",
    "Total_Tokens",
    "Context_Precision",
    "Answer_Relevance",
...
x = np.arange(len(context_eval_df))
labels = [f"Q{i+1}" for i in range(len(context_eval_df))]
...
comparison_df = pd.DataFrame({
    "Total_Tokens": _minmax(context_eval_df["Total_Tokens"]),
    "Context_Precision": _minmax(context_eval_df["Context_Precision"]),
    "Answer_Relevance": _minmax(context_eval_df["Answer_Relevance"]),
    "Faithfulness": _minmax(context_eval_df["Faithfulness"]),
    "Latency_ms": _minmax(context_eval_df["Latency_ms"]),
})

fig, ax = plt.subplots(figsize=(14, 6))
series_specs = [
    ("Total_Tokens", "#4C72B0"),
    ("Context_Precision", "#55A868"),
    ("Answer_Relevance", "#C44E52"),
    ("Faithfulness", "#8172B3"),
    ("Latency_ms", "#DD8452"),
]
...
ax.set_ylabel("Normalized value (min-max)")
ax.set_ylim(-0.05, 1.05)
ax.set_title(
    "Comparison of Total_Tokens, Context_Precision, Answer_Relevance, Faithfulness, and Latency_ms",
    fontweight="bold",
)
ax.spines["top"].set_visible(False)
ax.spines["right"].set_visible(False)
...
plt.savefig("context_eval_tokens_precision_relevance_latency.png", dpi=150, bbox_inches="tight")
plt.show()
print("Saved context_eval_tokens_precision_relevance_latency.png")
